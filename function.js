function checkbox() {
    document.getElementById("select-return-date").style.display(block);
}
...
function selectPlace() {
    var id=1;
    var listplace = '<select class="form-select" id="select-orign" aria-label="Default select example">';
    for (var i in tour) { 
        var data = JSON.parse(JSON.stringify(tour[i]));
        if (data.id == id) {
            listplace += '<option value="' + data.orign + '">'+ data.orign +'</option>';
            id++; 
        }     
    }
    listplace += '</select>';
...
function selectDestination() {
    var start = document.getElementById("select-orign"); 
    var starts = start[start.selectedIndex].value;
    var destination = '<select class="form-select" id="select-destination" aria-label="Default select example">';
    for (var i in tour) {
        var data = JSON.parse(JSON.stringify(tour[i]));
        if (starts == data.orign) {
            destination += '<option value="' + data.destination + '">' + data.destination + '</option>';  
        }
    }
    destination += '</select>';
    document.getElementById("listDistant").innerHTML = destination;
};
